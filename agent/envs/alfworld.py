...
class AlfWorldEnv(BaseEnv):
    ...
    def _maybe_parse_structured_str(self, value):
        ...
        # Attempt to parse as YAML (starts with '-', contains ':', or contains a newline)
        ...
        # Otherwise, retain the original string
        return value
    ...
    def _call_llm_and_parse(self, prompt: str, max_retries: int = 5):
        for attempt in range(max_retries):
            try:
                ...
                # 1. Initialize unified variables (prevents UnboundLocalError)
                extracted_thinking = ""
                parsed_data = None 
                
                # 2. Attempt parsing (prioritize JSON, fallback to YAML)
                try:
                    cleaned_json_str = self._extract_json_from_text(text_output)
                    parsed_data = json.loads(cleaned_json_str)
                except json.JSONDecodeError:
                    # JSON parsing failed, attempt YAML fallback
                    try:
                        cleaned_yaml_str = self._extract_yaml_from_text(text_output)
                        parsed_data = yaml.safe_load(cleaned_yaml_str)
                    except Exception:
                        pass # parsed_data remains None, which will correctly raise an error later in the flow

                if parsed_data is None:
                    raise ValueError("Failed to parse both JSON and YAML.")

                # 3. Intelligently strip the '_thinking' key and auto-unwrap the payload (core repair logic)
                if isinstance(parsed_data, dict):
                    # --- A. Extract the thinking process ---
                    ...
                    # --- B. Auto-Unwrap the payload ---
                    # At this point, parsed_data is sanitized.
                    # If only a single top-level key remains (e.g., "Current State") mapping to the desired List/Dict, extract its value directly.
                    keys = list(parsed_data.keys())
                    if len(keys) == 1:
                        # Overwrite parsed_data with the inner nested structure
                        parsed_data = parsed_data[keys[0]]
                
                # 4. Recursively parse internal structures (handling string representations of List/Dict elements)
                if isinstance(parsed_data, dict):
                    parsed_data = {
                ...
                # 5. Merge thinking strings and return
    ...
    def _get_fmt_schema(self, key: str) -> str:
        # Retrieve the base data structure definition
        structure, _ = self._get_format_config()
        
        # Construct a wrapper schema instructing the LLM to output its step-by-step thinking process
        wrapper_schema = {
            "_thinking": "Step-by-step analysis of the state changes...",
            key: structure
        }
        # Note: If your prompt's root key is already embedded in the structure, this could simply be:
    ...
    def get_CurState(self, action, observation, prev_states, goal_description, prev_goal_state) -> Tuple[Dict, str]:
        """
        Executes the 2-step process: Recall -> Evolve.
        Returns the FINAL Evolved State (CurState) and the combined thinking process.
        """
        ...
        # 2. Evolve Phase (The result of this IS the new CurState)
        prompt_update = PROMPT_TASK_RELATED_TEMPLATE.format(
            system_instruction="",
            output_format=self._get_fmt_schema("Previous States"), # Matches Prompt Template Key
            output_format_des=format_des,
            goal_description=goal_description,
            prev_states=str_prev_states,
        ...
        # Combine thinkings or just return the update thinking
    ...
    def get_GoalState(self, current_state, observation, action_obj, goal_description, prev_goal_state) -> Tuple[Dict, str]:
        """
        Extracts the Goal State based on the Evolved CurState.
        """
        _, format_des = self._get_format_config()
        str_prev_goal = json.dumps(prev_goal_state, ensure_ascii=False) if prev_goal_state else "None"
    ...
    def process_single_step(self, action, observation, accumulated_state, goal_description, prev_goal, step_idx):
        """
        Orchestrates the sequence: get_CurState -> get_GoalState
        """
        try:
            # --- Step 1: Get CurState (Recall + Evolve) ---
            ...
            # --- Step 2: Get GoalState (Immediately using the CurState) ---
    ...
    def generate_trajectory_data(self) -> Dict[str, Any]:
        ...
        for i in tqdm(range(len(gold_actions)), desc="Processing Trajectory"):
            ...
            # Format for final output (Normalize keys)
            # LLM usually returns keys like "Previous States" or "Goal State"
            # We want uniform keys "CurState" and "GoalState" in the dataset
            final_obj = {
                "Step": i,
                "Action": actions_and_obs.get("action", actions_and_obs),
                "Observation": actions_and_obs.get("observation", actions_and_obs),
                # "CurState": raw_cur.get("Previous States", raw_cur),
                # "GoalState": raw_goal.get("Goal State", raw_goal),
                "CurState": raw_cur,
                "GoalState": raw_goal
            }
            # print(f"CurState: {raw_cur.get('Previous States', raw_cur)}")
            # print(f"GoalState: {raw_cur.get('Goal State', raw_cur)}")
