...
class BlocksWorldEnv(BaseEnv):
    ...
    def _maybe_parse_structured_str(self, value):
        ...
        # Attempt to parse as YAML (starts with list dash, or contains a colon or newline)
    ...
    def _call_llm_and_parse(self, prompt: str, max_retries: int = 5) -> Tuple[Any, str]:
        for attempt in range(max_retries):
            try:
                ...
                # 1. Basic parsing (Prioritize JSON, fallback to YAML)
                try:
                ...
                if parsed_data is None:
                    raise ValueError("Failed to parse output from LLM.")

                # 2. Recursive cleaning function (synchronized with AlfWorldEnv logic)
                def deep_clean(data):
                    if not isinstance(data, dict):
                        return data, ""
                    think = ""
                    if "_thinking" in data:
                        think = data.pop("_thinking")
                    elif "thinking" in data:
                        think = data.pop("thinking")
                    
                    keys = list(data.keys())
                    if len(keys) == 1:
                        inner_data, inner_think = deep_clean(data[keys[0]])
                        return inner_data, (str(think) + "\n" + str(inner_think)).strip()
                    return data, think

                # 3. Execute cleaning and unwrapping
                parsed_data, extracted_thinking = deep_clean(parsed_data)

                # 4. Recursively parse internal structures
    ...
    def _get_fmt_schema(self, key: str) -> Dict:
        ...
        # Uniformly use the provided 'key' as the dictionary key without additional braces
        return {
    ...
    def generate_trajectory_data(self) -> Dict[str, Any]:
        ...
        for i in tqdm(range(len(gold_actions)), desc="Processing BlocksWorld Trajectory"):
            action = gold_actions[i]
            
            # BlocksWorld observation alignment logic
            obs = observations[i]
            ...
            # Format normalization (direct assignment since _call_llm_and_parse automatically unwraps the payload)
            final_obj = {
                "Step": i,
                "Action": action,
                "Observation": obs,
