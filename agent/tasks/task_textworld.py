...
def load_textworld_task(
    data_dir: str = "rewardprediction/textworld",
    part_num: int = 1,
    part_idx: int = -1
) -> Tuple[Generator[Dict[str, Any], None, None], int]:
    """
    Loads TextWorld trajectories from nested JSON files.
    ...
    if not traj_dir.exists():
        raise FileNotFoundError(f"TextWorld trajectory directory not found: {traj_dir}")
